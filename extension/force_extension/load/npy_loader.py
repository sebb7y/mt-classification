...
def load_settings(exp_folder):
    config_path = os.path.join(exp_folder, "config.yaml")
    if os.path.exists(config_path):
        with open(config_path, "r", encoding="utf-8") as f:
            settings = yaml.safe_load(f)
    else:
        settings = {"framerate": 58, "pixel_size": 100, "refIndices": -1, "processing_zcorrection": 0.88}  # defaults
    ...
    if isinstance(ref_val, (int, float)):
        ...
        ref_str = ref_indices[-1].strip().strip("'").strip('"') if ref_indices else ""
        try:
            reference_id = int(ref_str) if ref_str != "" else -1
        except (ValueError, TypeError):
            reference_id = -1
...
def load_npy_data(filepath, correction_factor=None, reference_id=None, framerate=None, already_processed=False, verbose=False):

    # from pyndoras box 

    
    exp_folder = os.path.dirname(os.path.abspath(filepath))
    settings, ref_from_config, correction_from_config, framerate_from_config = load_settings(exp_folder)
    ...
    if verbose:
        print("  config: framerate={}, correction_factor={}, reference_id={}".format(framerate, correction_factor, reference_id))
...
def load_and_plot_npy(filepath, n=6, seed=42, dot_size=1, verbose=True, focus_within_2std=False):
    if verbose:
        print("load_traces: built-in")
    exp_z, exp_time, exp_x, exp_y = load_npy_data(filepath, verbose=verbose)
    if verbose:
        t = exp_time.iloc[:, 0].values
        z = exp_z.values
        print(f"  shape: time={len(t)}, beads={exp_z.shape[1]}, time range [0, {t[-1]:.2f}] s")
        print(f"  z range [{np.nanmin(z):.4f}, {np.nanmax(z):.4f}] (µm)")
    if focus_within_2std:
        exp_z, exp_time, exp_x, exp_y, (start, end) = mask_within_n_std(exp_z, exp_time, exp_x, exp_y, n_std=2)
        if verbose:
            t = exp_time.iloc[:, 0].values
            z = exp_z.values
            print(f"  focus (≥95% beads within 2 std): indices [{start}, {end}), time range [{t[0]:.2f}, {t[-1]:.2f}] s, {len(t)} points")
            print(f"  z range [{np.nanmin(z):.4f}, {np.nanmax(z):.4f}] (µm)")
...
if __name__ == "__main__":
    ...
    if path and os.path.isfile(path):
        ...
        print("  --focus (or --within-2std): plot only the longest section where ≥95% of beads are within mean ± 2*std")
        print("  or: from force_extension.load.npy_loader import load_and_plot_npy; load_and_plot_npy('path/to/traces.npy', focus_within_2std=True)")
