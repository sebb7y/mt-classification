...
def main():
    path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(REPO_ROOT, "old_data", "txt_zainab_1")
    ...
    regions = exp.get("rotation_regions") or []
    print(f"rotation regions: {len(regions)}")
    for i, (t0, t1) in enumerate(regions):
        ...
        print(f"  rot {i}: [{t0:.2f}, {t1:.2f}] s  duration={t1-t0:.2f}s  in trace range? {in_range}")
    print()

    exts = build_exts_from_exp(exp, region_type="rotation")
    print(f"built {len(exts)} rotation extension(s)")
    for i, ext in enumerate(exts):
        reg = ext.get("region", [])
        gb = ext.get("gb_df")
        mag_rots = ext.get("mag_rots")
        n_pts = len(gb) if gb is not None else 0
        n_rot = len(mag_rots) if mag_rots is not None else 0
        print(f"  rot {i}: region [{reg[0]:.2f}, {reg[1]:.2f}] s  -> {n_pts} time points per bead, mag_rots length {n_rot}")
    return 0
